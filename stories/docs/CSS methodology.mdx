...
When we need to create an internal wrapper inside an element, let's avoid using the word `.wrapper` or `.container`. Instead, simply use this convention for internal wrappers (wrapper1, wrapper2, wrapper3, etc) `w1`, `w2`, `w3`.
For example:
